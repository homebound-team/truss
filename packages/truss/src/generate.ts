...
/**
 * For the "web" target, we generate a web-friendly CssBuilder whose public
 * TypeScript surface matches the runtime targets: `.$` is typed as the accumulated
 * style object shape, even though the build-time plugin later lowers it into className/style output.
 *
 * The actual web transformation happens at build time via the truss Vite plugin,
...
export const ${def("RuntimeCss")} = new CssBuilder<Pick<Properties, never>, "runtime">({
  rules: {} as Pick<Properties, never>,
  enabled: true,
  selector: undefined,
  elseApplied: false,
 ...
 */
function generateTrussMapping(config: Config, entries: WebEntry[]): TrussMapping {
